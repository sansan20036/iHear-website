...
const copy = {
  en: { heading: "Resources", intro: "Manage the forms and links listed on the Resources page. Names here do not change the original forms.", add: "Add resource", title: "Name", description: "Description (optional)", url: "HTTPS link", order: "Display order", edit: "Edit", hide: "Hide", publish: "Publish", trash: "Move to trash", confirm: "Remove this website link? The original form will remain available.", empty: "No resources yet.", retry: "Reload saved resources", draft: "Draft", published: "Published", cancel: "Cancel", save: "Save draft", savePublish: "Save and publish", auto: "Automatically update Chinese translations", force: "Also replace manually edited Chinese", preview: "Generate translation preview", ready: "Preview ready. Review the text, then save.", pending: "Working…", view: "View Resources", nameRequired: "Enter an English name.", conflict: "This resource changed elsewhere. Your draft is kept; close it and reload before saving." },
  zhHant: { heading: "資源管理", intro: "管理資源頁上的表單與連結。在此改名不會修改原始表單。", add: "新增資源", title: "名稱", description: "用途說明（選填）", url: "HTTPS 連結", order: "顯示順序", edit: "編輯", hide: "隱藏", publish: "發布", trash: "移至回收區", confirm: "移除此網站連結？原始表單仍會保留。", empty: "目前沒有資源。", retry: "重新讀取已儲存資料", draft: "草稿", published: "已發布", cancel: "取消", save: "儲存草稿", savePublish: "儲存並發布", auto: "自動更新中文翻譯", force: "同時覆蓋人工修改的中文", preview: "產生翻譯預覽", ready: "預覽已完成，請檢查文字後再儲存。", pending: "處理中…", view: "查看資源頁", nameRequired: "請填寫英文名稱。", conflict: "此資料已被其他管理員更新。您的草稿已保留，請關閉並重新讀取後再儲存。" },
  zhHans: { heading: "资源管理", intro: "管理资源页上的表单与链接。在此改名不会修改原始表单。", add: "新增资源", title: "名称", description: "用途说明（选填）", url: "HTTPS 链接", order: "显示顺序", edit: "编辑", hide: "隐藏", publish: "发布", trash: "移至回收区", confirm: "移除此网站链接？原始表单仍会保留。", empty: "目前没有资源。", retry: "重新读取已保存数据", draft: "草稿", published: "已发布", cancel: "取消", save: "保存草稿", savePublish: "保存并发布", auto: "自动更新中文翻译", force: "同时覆盖人工修改的中文", preview: "生成翻译预览", ready: "预览已完成，请检查文字后再保存。", pending: "处理中…", view: "查看资源页", nameRequired: "请填写英文名称。", conflict: "此数据已被其他管理员更新。您的草稿已保留，请关闭并重新读取后再保存。" },
};
const blank = (): ResourceInput => ({ title: { en: "", zhHant: "", zhHans: "" }, description: { en: "", zhHant: "", zhHans: "" }, url: "", sortOrder: 100, status: "draft" });
...
export default function ResourcesAdminPage() {
  ...
  function open(item?: ResourceLink) {
    setEditing(item || "new");
    setForm(item ? { title: structuredClone(item.title), description: structuredClone(item.description), url: item.url, sortOrder: item.sortOrder, status: item.status === "published" ? "published" : "draft" } : blank());
  ...
  return <section className="admin-page">
    ...
    {loading ? <p role="status">{text.pending}</p> : items.length ? <div className="admin-resource-list">{items.map(item => <article className="admin-card" key={item.id}>
      <h2>{item.title[locale] || item.title.en}</h2><p>{item.description[locale] || item.description.en}</p><a href={item.url} target="_blank" rel="noopener noreferrer">{item.url}</a><p>{text.order}: {item.sortOrder} · {item.status === "published" ? text.published : text.draft}</p>
