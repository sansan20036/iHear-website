...
確認管理員登入後即預載完整名冊；關閉管理模式只移除管理專用 DOM，保留本次登入的記憶體快取。再次開啟會同步顯示已載入的名冊，再背景核對最新資料。背景更新也同步此快取，版本衝突保護維持不變。首次讀取未完成時顯示「正在載入管理名冊…」，失敗提供重試。登出清除快取，晚到的回應不會重新填入；不使用 localStorage 或持久快取。
