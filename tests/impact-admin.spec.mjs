...
async function mockApplication(page, { admin = true, duplicateAvatar = false, tutorName = "Test Tutor", tutorFields = {} } = {}) {
  ...
  await page.route("**/api/site-media**", async (route) => {
    ...
    const imageName = slot.startsWith("impact.") && mediaItems[slot] ? "seminar" : "volunteers";
    const item = {
      slot,
      alt: isTutoring ? {
      ...
      src: `/assets/images/${imageName}-1200.webp`,
      srcSet: [480, 800, 1200].map(width => `/assets/images/${imageName}-${width}.webp ${width}w`).join(", "),
      variants: (isAvatar ? [480, 800] : [480, 800, 1200]).map((width) => ({
        ...
        byteSize: 1000,
        mimeType: "image/webp",
        url: `/assets/images/${imageName}-${width}.webp`,
      })),
    };
...
for (const chartPage of ["/", "/impact"]) {
  test(`${chartPage} charts support independent uploads, uncropped mobile images, reload and restore`, async ({ page }) => {
    const mocked = await mockApplication(page);
    ...
    await expect(learners).toHaveCSS("opacity", "1");
    await page.locator(".impact-grid").screenshot({ path: `output/impact-chart-upload/${chartPage === "/" ? "home" : "impact"}-original-with-upload-controls.png` });
    ...
    await learners.scrollIntoViewIfNeeded();
    await expect(learners).toHaveCSS("opacity", "1");
    await page.locator(".impact-grid").screenshot({ path: `output/impact-chart-upload/${chartPage === "/" ? "home" : "impact"}-uploaded-desktop.png` });
    for (const width of [390, 320]) {
      ...
      if (width === 390) await learners.screenshot({ path: `output/impact-chart-upload/${chartPage === "/" ? "home" : "impact"}-uploaded-mobile.png` });
    }
  ...
  test(`${chartPage} chart visitors cannot edit and never see old figures before a saved chart loads`, async ({ page }) => {
    await mockApplication(page, { admin: false });
    let release;
    const pending = new Promise(resolve => { release = resolve; });
    await page.route("**/api/site-media", async route => {
    ...
    await page.goto(chartPage, { waitUntil: "domcontentloaded" });
    const learners = page.locator('[data-site-media-slot="impact.learners.chart"]');
...
test("Home and Impact share chart uploads, replacements and restores in both directions", async ({ page }) => {
  const mocked = await mockApplication(page);
  const chart = await sharp({ create: { width: 800, height: 1000, channels: 3, background: "white" } }).png().toBuffer();
  const learners = page.locator('[data-site-media-slot="impact.learners.chart"]');
  const tutors = page.locator('[data-site-media-slot="impact.tutors.chart"]');
  const dialog = page.locator(".site-media-dialog[open]");
  ...
  async function restore(card) {
    ...
    await expect(card.locator("[data-site-media-fallback]")).toBeVisible();
  }

  await page.goto("/");
  await upload(learners);
  const firstLearners = structuredClone(mocked.getMediaItem("impact.learners.chart"));
  await page.goto("/impact");
  await expect(learners.locator("img")).toHaveAttribute("src", firstLearners.src);
  await expect(learners.locator("[data-site-media-fallback]")).toBeHidden();
  await upload(tutors);
  const savedTutors = structuredClone(mocked.getMediaItem("impact.tutors.chart"));
  await upload(learners);
  const replacement = mocked.getMediaItem("impact.learners.chart");
  expect(replacement.src).not.toBe(firstLearners.src);
  expect(replacement.recordVersion).toBeGreaterThan(firstLearners.recordVersion);
  await page.goto("/");
  await expect(learners.locator("img")).toHaveAttribute("src", replacement.src);
  await expect(tutors.locator("img")).toHaveAttribute("src", savedTutors.src);
  await restore(learners);
  expect(mocked.getMediaItem("impact.tutors.chart")).toEqual(savedTutors);
  await page.goto("/impact");
  await expect(learners.locator("[data-site-media-fallback]")).toBeVisible();
  await expect(tutors.locator("picture")).toBeVisible();
  await restore(tutors);
  await page.goto("/");
  await expect(learners.locator("[data-site-media-fallback]")).toBeVisible();
  await expect(tutors.locator("[data-site-media-fallback]")).toBeVisible();
  expect(mocked.getMediaUploadCount()).toBe(3);
  expect(mocked.requests.filter(request => /(?:POST|DELETE).*\/api\/site-media\//.test(request)))
    .toHaveLength(5);
});
...
test("all repository content photos expose stable sitewide media slots", async ({ page }) => {
  ...
  for (const slot of ["impact.learners.chart", "impact.tutors.chart"]) {
    await expect(page.locator(`[data-site-media-slot="${slot}"] .site-media-edit`)).toHaveCount(1);
  }
