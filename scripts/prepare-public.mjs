...
const clientAssetVersion = "20260920-navigation-animation-v1";
...
function withClientScripts(html, file) {
  ...
  if (html.includes("data-media-gallery")) scripts.push(`  <script src="/assets/gallery-video.js?v=${clientAssetVersion}" defer></script>`, `  <script src="/assets/media-galleries.js?v=${clientAssetVersion}" defer></script>`);
...
for (const file of htmlFiles) {
  ...
  const targetPath = path.join(file === "team.html" ? privateDir : publicDir, file);
  const html = await readFile(sourcePath, "utf8");
  verifyContentBuild(html, file, catalogIdentities);
  await writeFile(targetPath, withClientScripts(html, file), "utf8");
  // Public copies support the local legacy fixture. Production serves only the
  // private templates through current-data routes; .html URLs redirect there.
  if (file !== "team.html") await writeFile(path.join(privateDir, file), withClientScripts(html, file), "utf8");
}

for (const file of htmlFiles) {
  const html = await readFile(path.join(file === "team.html" ? privateDir : publicDir, file), "utf8");
