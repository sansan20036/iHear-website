...
export async function processSiteMediaImage(file: File): Promise<ProcessedSiteMediaVariant[]> {
  if (file.type !== "image/webp") {
    throw new SiteMediaImageError("The uploaded file must be a compressed WebP image");
  }
  if (file.size < 1 || file.size > CLIENT_IMAGE_MAX_BYTES) {
    throw new SiteMediaImageError("The compressed image must be smaller than 0.95MB", 413);
  }
  ...
  return Promise.all(SITE_MEDIA_WIDTHS.map(async (width) => {
